...
class ShortestForwarding(app_manager.RyuApp):
    ...
    def _state_change_handler(self, ev):
        ...
        if ev.state == MAIN_DISPATCHER:
            if not datapath.id in self.datapaths:
                ...
                root = Node(name=datapath.id)
                self.datapaths_entrytree.setdefault(datapath.id,RadixTree(root=root))
    ...
    def send_flow_mod(self, datapath, flow_info, src_port, dst_port):
        ...
        radix_tree = self.datapaths_entrytree.get(datapath.id)

        # node_list_copy = radix_tree.level_traverse()
        # node_list_copy.reverse()

        radix_tree.add(Node(prefix=ip_to_bin(ipv4_src),action=dst_port,in_port=src_port,eth_type=flow_info[0],ipv4_src=flow_info[1], ipv4_dst=flow_info[2]))
        match = parser.OFPMatch(
                    in_port=src_port, eth_type=flow_info[0],
                    ipv4_src=flow_info[1], ipv4_dst=flow_info[2])
        self.add_flow(datapath, 1, match, actions,idle_timeout=15, hard_timeout=60)

        aggregator = Aggregator(radix_tree)
        ...
        node_list_before_aggre = []
        for node in node_list:
            node_list_before_aggre.append(node)

        for node in node_list:
            aggregator.parent_child_aggr(node, node_list)

        for node in node_list:
            aggregator.sibling_aggr(node, node_list)

        for i in range(len(node_list)):
            if node_list_before_aggre[i].action != None and node_list_before_aggre[i].aggregated==False and node_list[i].action != None and node_list[i].aggregated == True:
                match = parser.OFPMatch(
                    in_port=node_list[i].src_port, eth_type=node_list[i].eth_type,
                    ipv4_src=node_list[i].ipv4_src, ipv4_dst=node_list[i].ipv4_dst)
                self.del_flow(datapath, match)



            if node_list_before_aggre[i].action == None and node_list_before_aggre[i].aggregated==False and node_list[i].action != None and node_list[i].aggregated == False:
                actions_add = []
                actions_add.append(parser.OFPActionOutput(node_list[i].action))
                match = parser.OFPMatch(
                    in_port=node_list[i].src_port, eth_type=node_list[i].eth_type,
                    ipv4_src=node_list[i].ipv4_src, ipv4_dst=node_list[i].ipv4_dst)
                self.add_flow(datapath, 1, match, actions_add,
